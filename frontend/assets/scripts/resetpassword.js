...
function showStep(stepName) {
    ...
    emailSentView?.classList.remove('active');
    resetSuccessView?.classList.remove('active');
...
function validatePasswordStrength(password) {
    ...
    const strength = checkPasswordStrength(password);
    if (strength.score < 2) return { valid: false, message: 'Password is too weak. Use a stronger password.' };
    return { valid: true, message: '' };
}

// ========================================
// API Calls — use /auth prefix so Vite proxy forwards to FastAPI
// ========================================
...
async function handleResetPassword(e) {
    ...
    if (result.success) {
        ...
        setTimeout(() => { window.location.href = '/src/views/login.html'; }, 3000);    } else {
        showError(result.message);
    }
}
...
function initializePage() {
    ...
    if (token) {
        resetToken = token;
        updateStepProgress(2);
        showStep('step2');  // ✅ Token present → go straight to Set New Password
    } else {
        showStep('step1');  // ✅ No token → show Request Reset form
    }
}
